...
export class ParentChainTransactionReceipt implements TransactionReceipt {
  ...
  /**
   * Get classic ParenttoChild messages created by this transaction
   * @param childProvider
   */
  public async getParentToChildMessagesClassic(
    childProvider: Provider
  ): Promise<ParentToChildMessageReaderClassic[]> {
    const network = await getChainNetwork(childProvider)
  ...
  /**
   * Get any ParenttoChild messages created by this transaction
   * @param ChainSignerOrProvider
   */
...
export class ParentChainEthDepositTransactionReceipt extends ParentChainTransactionReceipt {
  /**
   * Wait for the funds to arrive on Child
   * @param confirmations Amount of confirmations the retryable ticket and the auto redeem receipt should have
   * @param timeout Amount of time to wait for the retryable ticket to be created
   * Defaults to 15 minutes, as by this time all transactions are expected to be included on Child. Throws on timeout.
  ...
  > {
    const message = (await this.getEthDeposits(chainProvider))[0]
    if (!message)
...
/**
 * An ParentChainTransactionReceipt with additional functionality that only exists
 * if the transaction created a single call to an Child contract - this includes
 * token deposits.
 */
export class ParentChainContractCallTransactionReceipt extends ParentChainTransactionReceipt {
  /**
   * Wait for the transaction to arrive and be executed on Child
   * @param confirmations Amount of confirmations the retryable ticket and the auto redeem receipt should have
   * @param timeout Amount of time to wait for the retryable ticket to be created
   * Defaults to 15 minutes, as by this time all transactions are expected to be included on Child. Throws on timeout.
   * @returns The wait result contains `complete`, a `status`, an ParentToChildMessage and optionally the `ChildTxReceipt`.
   * If `complete` is true then this message is in the terminal state.
   * For contract calls this is true only if the status is REDEEMED.
   */
  public async waitForChild<T extends SignerOrProvider>(
    chainSignerOrProvider: T,
    confirmations?: number,
    timeout?: number
  ): Promise<
  ...
  > {
    const message = (
      await this.getParentToChildMessages(chainSignerOrProvider)
    )[0]
    if (!message)
      throw new ArbSdkError('Unexpected missing ParentToChild message.')
