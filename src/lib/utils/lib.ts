...
export async function getFirstBlockForL1Block({
  ...
  return binarySearchForL2Block(arbProvider, forL1Block, minL2Block, maxL2Block, getL1Block, allowGreater);
}

/**
 * Perform binary search to find the first L2 block that corresponds to a given L1 block number.
 * @param arbProvider - The Arbitrum provider.
 * @param forL1Block - The L1 block number to search for.
 * @param start - The start of the search range.
 * @param end - The end of the search range.
 * @param getL1Block - Function to get the L1 block number for a given L2 block.
 * @param allowGreater - Whether to allow the search to go past the specified `forL1Block`.
 * @returns The first L2 block that corresponds to the given L1 block number.
 */
async function binarySearchForL2Block(
  arbProvider: ArbitrumProvider,
  forL1Block: number,
  start: number,
  end: number,
  getL1Block: (forL2Block: number) => Promise<number>,
  allowGreater: boolean
