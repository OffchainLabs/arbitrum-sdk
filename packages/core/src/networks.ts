...
import { ARB1_NITRO_GENESIS_L2_BLOCK, ADDRESS_ZERO } from './constants'
...
export interface ArbitrumNetwork {
   ...
   * In case of a chain that uses an ERC-20 token from the parent chain as its native/gas token,
   * this is the address of said token on the parent chain.
   */
  nativeToken?: string
   ...
   * This is a temporary property and will be removed in future if Bold is widely adopted
   * and the legacy challenge protocol is deprecated.
   */
  isBold?: boolean
}
...
/**
 * @deprecated Use {@link TokenBridge} instead. This type exists for SDK v3 -> v4 migration.
 */
...
/**
 * @deprecated Use {@link ArbitrumNetwork} instead. This type exists for SDK v3 -> v4 migration.
 */
...
/**
 * Returns the Arbitrum chain associated with the given chain id.
 *
 * In core, this only accepts a `number` (chain id). The provider overload
 * lives in library packages (ethers5/ethers6/viem).
 *
...
/**
 * Asserts that the given network has a token bridge.
 */
export function assertArbitrumNetworkHasTokenBridge<
  T extends ArbitrumNetwork,
>(network: T): asserts network is T & { tokenBridge: TokenBridge } {
  if (
...
/**
 * Returns true if the network uses ETH as its native/gas token.
 */
export function isArbitrumNetworkNativeTokenEther(
  ...
  return (
    ...
    network.nativeToken === ADDRESS_ZERO
  )
}

/**
 * Get the Nitro genesis block for a given chain.
...
/**
 * Get the multicall address for a given chain id.
 * Synchronous — only uses in-memory network data.
 */
...
/**
 * Maps the old {@link L2NetworkTokenBridge} (from SDK v3) to {@link TokenBridge} (from SDK v4).
 */
