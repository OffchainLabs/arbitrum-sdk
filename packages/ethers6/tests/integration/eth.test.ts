/**
 * Integration tests for ETH deposit via ethers v6 adapter.
 *
 * Runs against a local arbitrum-testnode. The test:
 * 1. Gets deposit calldata from the SDK
 * 2. Signs and sends the transaction with ethers v6
...
describe('ETH deposit (ethers6 integration)', () => {
  ...
  it('deposits ETH from L1 to L2 and waits for completion', {
    ...
    // 2. Sign and send with ethers v6
    ...
    //    so we convert the ethers6 receipt and provider.
