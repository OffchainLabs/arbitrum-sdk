/**
 * L1→L3 teleportation functions for ethers v6 users.
 *
 * Functions that require a provider accept ethers v6 Provider and wrap it
...
// ---------------------------------------------------------------------------
// Wrapper types with ethers v6 Provider
// ---------------------------------------------------------------------------
...
export interface GetEthL1L3DepositRequestParams {
  ...
  amount: bigint
  from: string
  l2Provider: Ethers6Provider
  l3Provider: Ethers6Provider
...
export async function getEthL1L3DepositRequest(
  ...
  return coreGetEthL1L3DepositRequest({
    ...params,
    l2Provider: wrapProvider(params.l2Provider),
    l3Provider: wrapProvider(params.l3Provider),
  })
}
...
export async function getErc20L1L3DepositRequest(
  ...
  return coreGetErc20L1L3DepositRequest({
    ...params,
    l1Provider: wrapProvider(params.l1Provider),
    l2Provider: wrapProvider(params.l2Provider),
    l3Provider: wrapProvider(params.l3Provider),
  })
}
...
export async function predictL2ForwarderAddress(
  ...
  if (params.l2Provider) {
    coreParams.l2Provider = wrapProvider(params.l2Provider)
  }
  if (params.l1Provider) {
    coreParams.l1Provider = wrapProvider(params.l1Provider)
  }
